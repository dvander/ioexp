...
DevPollImpl::handleEvent(int fd)
{
  ...
  // If we're edge-triggered, remove this flag.
  if (transport->flags() & kTransportET) {
    if (Ref<IOError> error = rm_events_locked(transport, outFlag)) {
...
DevPollImpl::Poll(int timeoutMs)
{
  ...
  for (int i = 0; i < nevents; i++) {
    ...
    if (pe.events & POLLIN) {
      handleEvent<POLLIN, kTransportReading>(slot);
      if (isFdChanged(slot))
        continue;
    }
    ...
    if (pe.events & POLLOUT)
      handleEvent<POLLOUT, kTransportWriting>(slot);
  }
